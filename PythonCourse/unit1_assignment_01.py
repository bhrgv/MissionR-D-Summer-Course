...
def get_multiples_desc(number, count):
    ...
    """
    list=range(number,count*number+1,number)
    list.reverse()
    return list
...
def ascii_distance(first, second):
    ...
    """
    import string
    collection=list(string.printable)
    collection.sort()
    return abs(collection.index(first)-collection.index(second)) # use 2 builtin functions to get your work done.
...
def get_binary(number):
    ...
    """
    number=bin(number)
    return number[2:]# use a builtin and slice the result ...
