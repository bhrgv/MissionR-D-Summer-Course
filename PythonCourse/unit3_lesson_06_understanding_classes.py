...
def test_instances_state():
    ...
    except AttributeError as ne: # fill up the exception you got
        assert True

    try:
        age = r1.age
        assert False, "Should not reach here"
    except AttributeError as ne: #what exception do you get?
        assert True
...
class Person(object):
    ...
    def get_bmi_result(self):
        ...
        bmi=self.weight/(float(height)*float(height))
        if bmi >= 25:
            return "obese"
        elif bmi >=18.5:
            return "healthy"
...
-defining classes
-adding attributes
-changing class defination dynamically
"""